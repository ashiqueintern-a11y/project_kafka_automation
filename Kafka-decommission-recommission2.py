...
def get_broker_disk_paths_from_kafka(kafka_bin: str, bootstrap_servers: str, 
                                      logger: logging.Logger) -> Dict[int, List[str]]:
    """
    Get disk mount paths for all brokers using kafka-log-dirs.sh.
    ...
    Returns:
        Dict mapping broker_id to list of disk mount paths
    """
    try:
        cmd = [
        ...
        for broker_obj in data.get('brokers', []):
            ...
            # Extract unique disk paths (log directories)
            disk_paths = []
            for log_dir_obj in broker_obj.get('logDirs', []):
                log_dir = log_dir_obj.get('logDir')
                if log_dir and log_dir not in disk_paths:
                    disk_paths.append(log_dir)
            
            broker_disk_paths[broker_id] = disk_paths
            logger.debug(f"Broker {broker_id} disk paths: {disk_paths}")
        
        return broker_disk_paths
        
    except subprocess.TimeoutExpired:
        logger.error("kafka-log-dirs.sh timed out")
        return {}
    except Exception as e:
        logger.error(f"Error getting broker disk paths: {e}")
