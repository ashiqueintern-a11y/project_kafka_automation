...
def stop_kafka_broker(logger: logging.Logger) -> bool:
    """
    Stop Kafka broker service on local machine.
    
    Returns:
        True if successful or already stopped
    """
    logger.info("Stopping Kafka broker service...")
    ...
    try:
        ...
        if result.returncode == 0:
            logger.info("✓ Kafka broker stopped via systemctl")
            return True
    except Exception as e:
        logger.debug(f"systemctl stop failed: {e}")
    ...
    try:
        ...
        if result.returncode == 0:
            logger.info("✓ Kafka broker stopped via service command")
            return True
    except Exception as e:
        logger.debug(f"service stop failed: {e}")
    
    # Try kafka-server-stop.sh
    kafka_bin = auto_detect_kafka_bin()
    if kafka_bin:
        try:
            result = subprocess.run(
                [f"{kafka_bin}/kafka-server-stop.sh"],
                capture_output=True,
                text=True,
                timeout=60
            )
            logger.info("✓ Kafka broker stop command executed")
            # Wait a bit for shutdown
            time.sleep(5)
            return True
        except Exception as e:
            logger.debug(f"kafka-server-stop.sh failed: {e}")
    
    logger.error("Failed to stop Kafka broker - tried systemctl, service, and kafka-server-stop.sh")
    return False


def start_kafka_broker(logger: logging.Logger) -> bool:
    """
    Start Kafka broker service on local machine.
    
    Returns:
        True if successful
    """
    logger.info("Starting Kafka broker service...")
    ...
    try:
        ...
        if result.returncode == 0:
            logger.info("✓ Kafka broker started via systemctl")
            time.sleep(5)  # Give it a moment to start
            return True
    except Exception as e:
        logger.debug(f"systemctl start failed: {e}")
    ...
    try:
        ...
        if result.returncode == 0:
            logger.info("✓ Kafka broker started via service command")
            time.sleep(5)
            return True
    except Exception as e:
        logger.debug(f"service start failed: {e}")
    
    logger.error("Failed to start Kafka broker - tried systemctl and service commands")
    logger.info("Please start Kafka manually and re-run with --skip-broker-start")
    return False


def check_broker_status(logger: logging.Logger) -> bool:
    """
