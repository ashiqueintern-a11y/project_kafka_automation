...
class KafkaTopicCreator:
    ...
    def check_disk_space(self) -> bool:
        ...
        try:
            ...
            # Calculate disk usage per broker
            for broker_info in brokers:
                ...
                # Get filesystem info for the log directory (assuming first log dir)
                # This gives us total/available space, not just Kafka data
                log_dir_path = log_dir_paths[0] if log_dir_paths else None
                total_disk_gb = None
                ...
                if log_dir_path and log_dir_path != 'unknown':
                    try:
                        ...
                        used_disk_bytes = total_disk_bytes - available_disk_bytes
                        disk_used_pct = (used_disk_bytes / total_disk_bytes * 100) if total_disk_bytes > 0 else 0
                    except Exception as e:
                        self.logger.debug(f"Could not get filesystem stats for {log_dir_path}: {e}")
            ...
            # Display results
            for stats in broker_stats:
                broker_id = stats['broker_id']
                kafka_gb = stats['kafka_data_gb']
                
                if stats['has_error']:
                    continue  # Already logged error
                
                # Show Kafka data size
                if stats['disk_used_pct'] is not None:
                    # We have filesystem info
                    self.logger.info(
                        f"Broker {broker_id} ({stats['log_dir']}): "
                        f"Kafka data: {kafka_gb:.2f}GB | "
                        f"Disk: {stats['available_disk_gb']:.1f}GB free / {stats['total_disk_gb']:.1f}GB total "
                        f"({stats['disk_used_pct']:.1f}% used)"
                    )
                    
                    # Check thresholds
                    if stats['available_disk_gb'] < 10:
                        self.logger.warning(
                            f"Broker {broker_id} - LOW DISK SPACE: Only {stats['available_disk_gb']:.1f}GB available"
                        )
                        total_issues += 1
                    elif stats['disk_used_pct'] > 85:
                        self.logger.warning(
                            f"Broker {broker_id} - HIGH DISK USAGE: {stats['disk_used_pct']:.1f}% used"
                        )
                        total_issues += 1
                else:
                    # No filesystem info, just show Kafka data
                    self.logger.info(
                        f"Broker {broker_id}: Kafka data size: {kafka_gb:.2f}GB"
                    )
            ...
            self.logger.warning("Raw output (first 500 chars):")
            self.logger.warning(stdout[:500])
            self.logger.warning("This might be a Kafka version compatibility issue")
            self.logger.info("Skipping disk space check - continuing with topic creation")
